...
def init_model(model_class, included_MDC=included_MDC):
    ...
    for param in model_class.parameter_names + model_class.parameters_stratified_names:
        #if isinstance(global_params_dict[param],np.ndarray) and len(global_params_dict[param])==1:
        #    params_dict.update({param:global_params_dict[param][0]})
        #else:
...
if __name__ == '__main__':
    ...
    bounds_global = [(0,100),(0,100),(0,1),(0,1),(50,1000)]
    ...
    for model_class in model_classes:
        ...
        for MDC_key in included_MDC:
            ...
            ndim, nwalkers, pos = perturbate_theta(theta, pert=[0.1,]*len(bounds), multiplier=multiplier_mcmc, bounds=bounds, verbose=True)
            ...
            #else:
            #    progress = True
            sampler = run_EnsembleSampler(pos, max_n, identifier+'_'+model_name+'_'+MDC_key, objective_function, (), {'simulation_kwargs':{'method': 'LSODA'}},
            ...
            if sampler.get_chain(discard=int(n_mcmc*3/4), thin=thin, flat=True).shape[0] > 20:
                ...
                discard = 0
            samples_dict[model_name].update({MDC_key:emcee_sampler_to_dictionary(discard=discard, thin=thin, samples_path=samples_path+model_name+'/',identifier=identifier+'_'+model_name+'_'+MDC_key)})
    ...
    for model_class in model_classes:
        ...
        # Simulate model
        N=5
        out_mcmc = model.sim([start_date, end_date], N=N, samples=samples_per_MDC, draw_fcn=draw_fcn, processes=processes,method='LSODA')
